    ...
    uint8_t proof[PROOF_LEN])
{
  ...
  {
    res = memcmp(public_keys[i], secret_key + crypto_sign_ed25519_SEEDBYTES,
                 crypto_sign_ed25519_PUBLICKEYBYTES);
    if (res != 0) continue;
  ...
  memcpy(proof, previous_commit, crypto_hash_sha512_BYTES);

  // Second part is the first public key in the ladder which will sign the proof
  memcpy(&proof[crypto_hash_sha512_BYTES], public_keys[required_key_index],
         crypto_sign_ed25519_PUBLICKEYBYTES);
  ...
  for (i = 1; i < IDENTITIES_LEN; i++)
  {
    ...
    res = crypto_auth_hmacsha512(hash_1, public_keys[i],
